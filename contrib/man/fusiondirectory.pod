...
FusionDirectory have lots of options that can alter the way it works.
All those parameters are configured with the help of the configuration plugin inside FusionDirectory and saved into the ldap tree.
...
>>>>>>> 5c7246b... Fixes: #2418 Removed gosaUnitTag
...
A pattern can include attribute names prefaced and terminated by a % and normal text:
...
The idGenerator statement describes an automatic way to generate new user ids.
